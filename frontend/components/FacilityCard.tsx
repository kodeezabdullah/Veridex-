...
export function FacilityCard({ facility, queriedCapability }: { facility: Facility; queriedCapability: string }) {
  ...
  return (
    <article className="facility-card">
      <div className="facility-card-top"><span className={`claim-badge ${claim.status}`}>{claim.status.replace("_", " ")}</span><span className="trust-score"><strong>{claim.trust_score_pct}</strong>/100 trust</span></div>
